...
def model():
    ...
    data = sio.loadmat("simulation_data_without_disturbance_1s_rest.mat", squeeze_me=True, struct_as_record=False)
    # True measurements
    ...
    #TODO: o1, o2 und einheitsvektoren in c1 (norm_1)
    """
    --------------------------------------------------------------------------
    ...
    # quatFromGyr1 = if_else(norm_2(gyr1_real) < 1e-6, NP.array([1,0,0,0]),quaternionFromGyr(gyr1_real, rate))
    # quatFromGyr2 = if_else(norm_2(gyr2_real) < 1e-6, NP.array([1,0,0,0]),quaternionFromGyr(gyr2_real, rate))
    # dquat1 = quaternionMultiply(quat1, quatFromGyr1)
    # dquat2 = quaternionMultiply(quat2, quatFromGyr2)
    ...
    x_lb = -1 * NP.ones(_x.shape[0])
    x_ub =  1 * NP.ones(_x.shape[0])

    x_lb[8:11]  = -2 * NP.ones(3) * 10
    x_lb[11:14] = -2 * NP.ones(3) * 10
    x_lb[14:17] = -10* NP.ones(3) * 10
    x_lb[17:20] = -10*NP.ones(3) * 10

    x_ub[8:11]  = 2 * NP.ones(3) * 10
    x_ub[11:14] = 2 * NP.ones(3) * 10
    x_ub[14:17] = 10* NP.ones(3) * 10
    x_ub[17:20] = 10*NP.ones(3) * 10
    # No algebraic states
    z_lb = NP.array([])
    z_ub = NP.array([])
    ...
    u0 = NP.squeeze(vertcat(acc1_[150,:], gyr1_[150,:], acc2_[150,:], gyr2_[150,:]))
    ...
    penalty_term_cons = NP.array([1e4,1e4,1e4,1e4])
    # Maximum violation for the constraints
    maximum_violation = NP.array([0.01,0.01,0.01,0.01])
