...
# Load data
path_to_data_1 = 'the_delta_u_situation/'
...
# Format is: |states|controls|
raw_data.append(NP.load(path_to_data_1+ "map_25" + ".npy"))
...
states.append(raw_data[0][:,:nx])
controls.append(raw_data[0][:,nx:])

# training data
n_train = 8000
x_train_1 = NP.vstack(states[0][0:n_train])
y_train_1 = NP.vstack(controls[0][0:n_train])

# test data
x_test_1 = NP.vstack(states[0][n_train:])
y_test_1 = NP.vstack(controls[0][n_train:])
...
model.fit(x_train_1, y_train_1,epochs=3000,batch_size=100)
pdb.set_trace()
