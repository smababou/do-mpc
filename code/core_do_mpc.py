...
class observer:
    ...
    def __init__(self, model_observer, param_dict, *opt):
        ...
        self.t_step = param_dict["t_step_observer"]
        self.t_cur = 0.0
        if not self.method == 'state-feedback':
            x = model_observer.x
            u = model_observer.u
            p = model_observer.p
            y = model_observer.y
            tv_p = model_observer.tv_p
            self.param_est = param_dict["parameter_estimation"]
            self.open_loop = param_dict["open_loop"]
            self.meas_fcn = Function('meas_fcn',[x,u,p,tv_p],[y])
            self.meas_scaling = model_observer.ocp.y_scaling
...
class configuration:
    ...
    def make_step_observer(self):
        observer_do_mpc.make_step_observer(self)
    ...
    def store_est_data(self):
        self.observer.t_cur += self.observer.t_step
        data = self.mpc_data
        nx = self.model.x.size(1)
        np = self.model.p.size(1)
        if self.observer.method == "EKF":
