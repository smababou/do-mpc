...
def setup_mhe(model, observer):
    ...
    P_inputs = observer.P_inputs
    P_param = observer.P_param
    P_meas = observer.P_meas
    # NOTE: Maybe add scaling
    J_states = mtimes(mtimes((x_est_past-x).T,P_states),x_est_past-x)
    J_inputs = mtimes(mtimes((u_est_past-u).T,P_inputs),u_est_past-u)
    J_meas = mtimes(mtimes((y_meas-y).T,P_meas),y_meas-y)
    J_param = mtimes(mtimes((p_est_past-p).T,P_param),p_est_past-p)
